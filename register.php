...
<head>
    ...
    <title>Les Privat - Register</title>
...
<body class="bg-gradient-primary">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        swal.fire({
            title: "Error",
            text: "Alamat email sudah terdaftar. Gunakan alamat email lain.",
            icon: "error",
            confirmButtonText: "OK",
        });
    </script>';
        } elseif (mysqli_stmt_num_rows($checkUsernameStmt) > 0) {
            echo '<script>
        swal.fire({
            title: "Error",
            ...
            $sql = "INSERT INTO users (email, username, role, password) VALUES (?, ?, ?, ?)";
            $stmt = mysqli_prepare($conn, $sql);
            mysqli_stmt_bind_param($stmt, "ssss", $email, $username, $role, $password);

            if (mysqli_stmt_execute($stmt)) {
                if ($role == 'guru') {
                    $sqlG = "INSERT INTO guru (username, email) VALUES (?, ?)";
                    $stmtG = mysqli_prepare($conn, $sqlG);
                    mysqli_stmt_bind_param($stmtG, "ss", $username, $email);
                    mysqli_stmt_execute($stmtG);
                } elseif ($role == 'murid') {
                    $sqlM = "INSERT INTO murid (username) VALUES (?)";
                    $stmtM = mysqli_prepare($conn, $sqlM);
                    mysqli_stmt_bind_param($stmtM, "s", $username);
